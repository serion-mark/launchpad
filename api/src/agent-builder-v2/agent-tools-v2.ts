...
import { AGENT_TOOL_TIMEOUT_MS } from '../agent-builder/stream-event.types';
...
export const AGENT_TOOLS = [
  ...
  {
    name: 'provision_app_v2',
    description:
      'Foundry 자체 NCP+Postgres+Prisma 백엔드 자동 설정 (Supabase 미사용). 답지에서 "자체 백엔드 (로그인 + DB)" 옵션을 고른 경우 호출. Postgres schema 생성 + Prisma init + .env.local 자동 작성 (DATABASE_URL/JWT_SECRET) + 다음 단계 안내. 사용자에게 추가 질문 금지.',
    input_schema: {
      type: 'object' as const,
      properties: {
        prismaSchema: {
          type: 'string',
          description: 'Prisma 모델 정의 텍스트 (User 모델 필수, bcrypt password 필드 포함). generator/datasource 블록은 도구가 자동 추가하니 model 부분만 작성.',
        },
      },
      required: ['prismaSchema'],
    },
  },
  {
...
export class AgentToolExecutor {
    ...
    try {
      ...
      switch (name) {
        ...
        case 'Grep':
          ...
          break;
        case 'provision_app_v2':
          output = await this.provisionAppV2(input);
          break;
  ...
  // provision_app_v2 — Foundry 자체 백엔드 자동 설정 (Supabase 미사용)
  // 1) Postgres 에 CREATE SCHEMA (멀티테넌트 격리)
  // 2) JWT_SECRET 자동 생성 (32 bytes hex)
  // 3) prisma/schema.prisma 자동 작성 (generator + datasource + 사용자 모델)
  // 4) .env.local 자동 작성 (DATABASE_URL with schema, JWT_SECRET, APP_SCHEMA)
  // 5) 다음 단계 안내 (npx prisma db push → 회원가입 API → 로그인 API)
  //
  // ⚠️ 운영 launchpaddb 의 schema 분리 사용. 본체 데이터 영향 0.
  private async provisionAppV2(input: any): Promise<string> {
    if (!this.deps.projectId) {
      throw new Error('projectId 없음 (startProject 선행 필요)');
    }
    if (!this.deps.prisma) {
      throw new Error('Prisma 서비스 미주입 (AgentBuilderV2Service 구성 확인)');
    }
    const prismaSchema = String(input?.prismaSchema ?? '').trim();
    if (!prismaSchema) throw new Error('prismaSchema 필수 (User 모델 포함)');

    // 1) Postgres schema 이름 (8자 cuid 기반, 안전한 식별자)
    const appId = this.deps.projectId.slice(-8).replace(/[^a-z0-9]/gi, '_').toLowerCase();
    const schemaName = `app_${appId}`;

    // 2) Postgres 에 CREATE SCHEMA 실행 (idempotent)
    try {
      await this.deps.prisma.$executeRawUnsafe(
        `CREATE SCHEMA IF NOT EXISTS "${schemaName}"`,
      );
    } catch (err: any) {
      throw new Error(`schema 생성 실패: ${err?.message}`);
    }

    // 3) JWT_SECRET 자동 생성
    const jwtSecret = crypto.randomBytes(32).toString('hex');

    // 4) DATABASE_URL 구성 (메인 DB + schema 분리)
    const mainDbUrl = process.env.DATABASE_URL || '';
    if (!mainDbUrl) {
      throw new Error('DATABASE_URL 환경변수 미설정 (Foundry 본체 .env 확인)');
    }
    const appDbUrl = mainDbUrl.includes('?')
      ? `${mainDbUrl}&schema=${schemaName}`
      : `${mainDbUrl}?schema=${schemaName}`;

    // 5) 프로젝트 루트에 .env.local 작성
    const root = await this.findProjectRoot();
    const envPath = path.join(root, '.env.local');
    const envContent =
      `# Foundry v2 자체 백엔드 (Supabase 미사용)\n` +
      `DATABASE_URL=${appDbUrl}\n` +
      `JWT_SECRET=${jwtSecret}\n` +
      `APP_SCHEMA=${schemaName}\n`;
    await fs.writeFile(envPath, envContent, 'utf8');

    // 6) prisma/schema.prisma 자동 작성
    const prismaPath = path.join(root, 'prisma', 'schema.prisma');
    await fs.mkdir(path.dirname(prismaPath), { recursive: true });

    const fullSchema =
      `// Auto-generated by Foundry v2 (provision_app_v2)\n` +
      ...
      `}\n\n` +
      prismaSchema;
    await fs.writeFile(prismaPath, fullSchema, 'utf8');

    // 7) Foundry DB 의 projects 테이블에 schema 정보 저장 (선택, 향후 cleanup 용)
    try {
      await this.deps.prisma.project.update({
        where: { id: this.deps.projectId },
        data: {
          // supabaseProjectRef 컬럼 재활용 (schema name 저장)
          // 운영 영향 X — v2 만 이 컬럼에 schema 이름 씀
    ...
    return (
      ...
      `- Postgres schema: ${schemaName} (CREATE SCHEMA 완료)\n` +
      `- prisma/schema.prisma 작성됨 (User 모델 + 사용자 정의 모델)\n` +
      `- .env.local 자동 주입 (${path.relative(this.cwd, envPath)})\n` +
      `  • DATABASE_URL=postgres://...?schema=${schemaName}\n` +
      `  • JWT_SECRET=${jwtSecret.slice(0, 16)}... (32 bytes hex)\n` +
      `  • APP_SCHEMA=${schemaName}\n` +
      `\n` +
      `📋 다음 단계 (Claude 가 자동 진행):\n` +
      `1. \`npx prisma generate\` 로 Prisma client 생성\n` +
      `2. \`npx prisma db push\` 로 테이블 생성 (schema 적용)\n` +
      `3. src/lib/prisma.ts 작성 (PrismaClient 싱글톤)\n` +
      `4. src/lib/auth.ts 작성 (JWT + bcrypt 헬퍼)\n` +
      `5. src/app/api/auth/signup/route.ts 작성\n` +
      `6. src/app/api/auth/login/route.ts 작성\n` +
      `7. 페이지 작성 (login/signup/dashboard)\n` +
      `\n` +
      `⚠️ Supabase 0% 사용. 모든 인증/DB = Prisma + JWT + bcrypt 자체 구현.\n` +
      `📚 패턴 참조: prompts/patterns/auth-jwt.md, prompts/patterns/nextjs-api-route.md\n`
    );
  }
