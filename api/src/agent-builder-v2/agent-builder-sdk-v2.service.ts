...
} from '../agent-builder/stream-event.types';
import { adaptSDKMessage } from '../agent-builder/sdk-message-adapter';
import { createFoundryMcpServer, FOUNDRY_MCP_TOOL_NAMES } from './sdk-tools-v2';
...
@Injectable()
export class AgentBuilderSdkV2Service {
  private readonly logger = new Logger(AgentBuilderSdkV2Service.name);
    ...
    try {
      ...
      for await (const sdkMsg of q) {
        ...
        const evs = adaptSDKMessage(sdkMsg, {
          ...
          stagePercent: AgentBuilderSdkV2Service.STAGE_PERCENT,
        });
        for (const ev of evs) {
