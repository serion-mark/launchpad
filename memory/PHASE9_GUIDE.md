...
3. **구조화된 입력 유도 UI** — 사용자가 구분 기호를 자연스럽게 쓰도록:
   - 플레이스홀더 예시: "반려동물 돌봄 앱 - 매칭, 예약, 리뷰 (펫시터 전용)"
   - 또는 기능 태그 입력 UI (태그 칩으로 기능 추가)
   - AI 결과 품질이 사용자 입력의 구조화 수준에 비례하므로
     UI 레벨에서 구조화를 유도하는 것이 핵심
   - 참고: `[], (), -` 구분 기호만 써도 AI 이해도 3배 향상 (대표 실사용 검증)
