...
**개선안 (기본 모델 자동 선택 — 추천)**:
- 모델 미선택 시 → Smart(Sonnet) 자동 선택
- 상단 모델 드롭다운에 기본값 세팅: `useState('smart')` 또는 `useState('sonnet')`
- 사용자가 원하면 Flash/Smart 변경 가능

**대안 (모델 선택 필수 안내)**:
- 모델 미선택 + "앱 생성하기" 클릭 → "AI 모델을 선택해주세요" 토스트 표시
- 또는 생성 버튼 비활성화 + 모델 드롭다운 하이라이트

**파일**: `web/src/app/builder/page.tsx` — 모델 선택 state 초기값 확인
