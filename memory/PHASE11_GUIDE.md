...
### /start 자연어 입력창 — 대화 모드 미작동
**현재 문제**: 자연어 입력창에 "뭘 할 수 있어" 같은 질문을 입력하면, AI가 대화로 답하지 않고 바로 custom 템플릿 문답 페이지로 리다이렉트됨. 입력 텍스트가 매장 이름 필드에 들어감.

**기대 동작**:
```
사용자: "뭘 할 수 있어?"
AI: "이런 앱을 만들 수 있어요: 예약앱, POS, 쇼핑몰..."
사용자: "반려동물 돌봄 앱 만들어줘"
AI: "좋아요! 몇 가지 질문할게요..."
→ 자연스럽게 문답으로 전환
```
...
**개선안**:
- 입력 텍스트를 AI에게 먼저 보내서 의도 파악
- 앱 생성 의도 → custom 문답으로 전환 (현재 동작 유지)
- 질문/대화 의도 → 채팅 응답 표시 후 앱 생성 유도
- 또는 더 심플하게: 입력창 placeholder를 "만들고 싶은 앱을 설명해주세요"로 변경하여 용도를 명확히

**파일**: `web/src/app/start/page.tsx` — 입력 submit 핸들러
