...
### Phase 6 ✅ 코드 품질 근본 개선 + UX 핵심 기능 (3/21)
- [x] **AI 코드 생성 지침서 프롬프트 주입** — FRONTEND/GENERATE/fixBuildErrors 3곳에 Foundry Static Export 규칙서 주입
  - 금지: next/headers, supabase/server, middleware.ts, [id] 동적라우트, Server Components, fetch('/api')
  - 필수: 'use client', .tsx 확장자, Supabase 클라이언트 전용
  - 대안패턴: 동적라우트 대신 useState+모달 상세보기 (코드 예시 포함)
- [x] **Tailwind CSS 빌드 보장** — postcss.config.mjs + @tailwindcss/postcss 자동 생성, package.json/deploy에서도 보장
- [x] **Sonnet 모델 활성화** — smart/pro 모델 available: true, ModelSelector UI에서 선택 가능
- [x] **동적 라우트 근본 해결** — 프롬프트에서 금지 + 클라이언트 대안패턴 주입 (deploy에서 제거는 안전장치로 유지)
- [x] **실시간 미리보기 (LivePreview)** — 생성 완료 후 Tailwind CDN + JSX→HTML 변환으로 빌드 없이 즉시 렌더링
  - 페이지 탭 네비게이션, 모바일/PC 전환, sandbox iframe
- [x] **Static Export 호환성 개선** — server.ts/middleware.ts/auth callback route 생성 제거, 클라이언트 전용 구조

### Phase 7 🔄 고급 기능 (진행 중 3/21)
- [x] **코드 수정 스마트 선별** — smartFileSelection() 키워드 매칭으로 관련 파일 자동 타겟팅
- [x] **사이드바 네비게이션 자동 생성** — Sidebar.tsx 컴포넌트, pages[]로 메뉴 구성 + 인증 + 로그아웃
- [ ] **범용 템플릿 추가** — "자유롭게 만들기" 템플릿 (Lovable처럼 업종 제한 없이)
...
Phase 7 이어서 해줘. memory/MEMORY.md 참고.
- [완료] Phase 1~6 + Phase 7 일부 (스마트선별+사이드바 커밋 완료)
- [현재] Phase 7 남은 작업:
  1. 범용 템플릿 추가 — Lovable처럼 업종 제한 없이 "자유롭게 만들기"
     (현재 6종: 미용실/CRM/커머스/O2O/에듀/시설 → +범용 추가)
  2. E2E 테스트 — Phase 6 프롬프트 개선 효과 검증 (Tailwind CSS 적용 확인)
  3. 세리온 모듈 RAG 주입
