...
### Phase 9 ✅ 기본기 완성 (3/22, 반나절 만에 완료!)
- [x] 빌드 성공률 80%+ (패키지 화이트리스트+사전검증+에러패턴 30개)
- [x] 실시간 미리보기 (SSE→LivePreview)
- [x] 자연어 자유 입력 (/start 상단 대화창)
- [x] 부분 수정 (스마트 선별 25패턴)
- [x] 테마 중앙 관리 (CSS 변수 9토큰 + 7프리셋)
...
#### Phase 11 추가 완료 (3/23)
- [x] **Gemini 429 재시도** — callGoogle() 사전 2초 딜레이 + 429/RESOURCE_EXHAUSTED 감지 → 10s/20s/30s 백오프 최대 3회 retry
- [x] **AI 회의실 사전 질문 단계** — POST /ai/meeting-pre-question (Haiku), 사전 질문 UI → 답변 입력(선택) → 회의 시작 시 preAnswers 컨텍스트 주입
- [x] **PDF 파서 서버사이드** — pdf-parse 라이브러리, POST /ai/parse-pdf (FileInterceptor), 프론트 PDF accept 추가, 서버에서 텍스트 추출
- [x] **에러 메시지 사용자 친화적 변환** — AllExceptionsFilter에 상태코드별 한글 메시지 매핑, 한글 에러는 그대로 전달, 프론트 friendlyMessages 추가
- [x] **React hydration 에러 #418** — getToken() 직접 렌더링 → useEffect+useState(isLoggedIn) 패턴으로 전환, SSR/CSR 불일치 해소
...
### Phase 12 ✅ 완성도 강화 + 법적 보호 (3/23, Day 1~5 하루 만에 완료!)
- [x] CSS: Tailwind CDN fallback + CSS 변수 주입 (Turbopack 호환 문제 → CDN 유지)
- [x] Supabase DB: /database/query API + DDL/DML 분리 + 샘플 데이터 자동 삽입
- [x] 온보딩: /start 리다이렉트 + 크레딧 환산 배너
- [x] 마크다운 렌더링 (react-markdown + remark-gfm)
- [x] AI 존칭 통일 (~습니다/~드리겠습니다)
- [x] 기술 용어 순화 (Lock-in→종속 없이, 서브도메인→전용 주소)
- [x] 크레딧 충전제 3탭 UI (충전/모두의창업/독립)
  - 충전: 5,000cr(49k) / 20,000cr(149k) / 50,000cr(249k) + 소량(1k/3k)
  - 모두의창업: 라이트(490k/월) / 스탠다드(990k/월)
  - 독립: 코드팩(990k) / 프로팩(1,990k) / 엔터프라이즈(4,990k)
- [x] /pricing 가격표 동기화, /terms 면책+데이터보관, /refund 환불 강화
- [x] deploy.sh web npm install 추가 (502 방지)
- [x] 메인 페이지 fallback (404 방지), 영어 메뉴 한글화
- 상세: memory/PRICING_REPORT_2026-03-23.md

### Phase 13 ✅ 보안 + 빌드큐 + QA (3/23, 완료!)
- [x] QA 3라운드: 6.5 → 8.0 → 8.4 → 9.1점
- [x] 보안 4건: 크레딧 Race Condition, Path Traversal, CORS 화이트리스트, 결제금액 검증
- [x] 크레딧 차감 구현 (회의실/분석/이미지 — 무한 무료 차단!)
- [x] 비밀번호/약관 서버 검증, SSE 메모리 누수 수정
- [x] 빌드 큐 (동시 2개 제한, in-memory)
- [x] OG 메타태그 + 모바일, 호스팅 가격 29,000원 통일
- [x] 스마트 분석 결과 화면 표시, 앱 유형별 필수 구조 프롬프트
- [x] /start→/builder 질문지 중복 수정 (answers+readyToGenerate 플래그)
- [x] 앱 생성 대기 중 AI 채팅 기능, 시간 문구 15분→30분
- ⚠️ **미해결: 미리보기 하드코딩 목업** (어떤 앱이든 매출 대시보드 나옴!)
- ⚠️ **미해결: 대기 채팅 프로젝트 정보 미연동** (채팅 AI가 뭘 만드는지 모름)
- 상세: memory/PHASE13_REPORT.md, PHASE13_HOTFIX_REPORT.md, PHASE13_FLOW_AUDIT.md

### 세션5 (mk5) 완료 작업 ✅ (2026-03-28)
- [x] **모두의창업 1,014개 아이디어 전수 분석** — 키워드/트렌드/Foundry 적합도
- [x] **Gemini 딥리서치** — 프롬프트 작성+실행+결과 수신 (117줄 보고서)
- [x] **Foundry 현재 상태 진단** — 기술80%/증거20%/안정성60%
- [x] **백설공주 사과농장 데모앱** — AI 회의실→앱 생성→배포 (app-7063.foundry.ai.kr)
- [x] **크레딧 10,000 충전** (2,061→12,061)
- [x] **버그 수정: "이걸로 앱 만들기"** — meeting_context→/start 자동 전달 (커밋 e7bfaa2)
- [x] **버그 수정: LIVE 미리보기 404** — nginx frame-ancestors에 *.foundry.ai.kr 추가
- [x] **버그 수정: text-white 안 보임** — 전체 HTML에 CSS !important 주입 (임시, CSS 빌드 해결 시 근본 수정)
- [x] **성능: tsc 사전 검증** — 빌드 전 3초 문법 체크, F6 개입 최소화 (커밋 0895bc3)
- [x] **데모앱 공장 시스템 구축** — memory/DEMO_APP_FACTORY.md (지시서+레이아웃6패턴+색상7종)

### 할 일 / 전략 / 플랜
- **상세**: `memory/FOUNDRY_PLAN_2026-03-28.md` 참고 (실행 순서, 기술 고도화, 전략)

### Phase 14 ❌ 120% 차별화 (다음!)
- [ ] 풀스택 백엔드 강화 (NestJS 자동생성)
- [ ] 한국 특화 자동연동 (카카오/네이버/토스)
- [ ] 템플릿 마켓플레이스
- [ ] 세리온급 복잡 앱 생성
- [ ] 독립 패키지 ZIP 생성 로직
...
### Phase 15 ❌ 유니콘 (장기)
- 글로벌 (영어/일본어), React Native, Fine-tuned 모델, 엔터프라이즈, API 플랫폼

### Foundry 특허 포트폴리오
- **1호**: 동적 슬롯 스케줄링 (세리온) — 출원 완료 (10-2026-0051041)
- **2호**: 풀스택 자동 생성 — 보류
- **3호**: 빌드 에러 AI 자동수정 — 기술설명서 완료 (/Users/mark/Desktop/세리온 전략/)
- **4호**: 순차 누적형 멀티AI 브레인스토밍 — 기술설명서 완료, 변리사 전달
  - ⚠️ 변리사 피드백: "AI 활용"이면 거절 가능 → "분석 모듈+알고리즘"으로 재작성 필요
  - 기술설명서 수정 명령어 준비 완료 (노션)
- **범용 설계**: 모든 특허를 특정 업종/플랫폼 한정 X로 설계
- 상세: /Users/mark/Desktop/세리온 전략/특허_포트폴리오.md
...
- 배포 전 tsc --noEmit 0 에러 확인!

## 토스페이먼츠
- MID 1: bill_serioi8u1 (빌링/자동결제) — 심사 중 (3/20 접수, 회신 필요!)
- MID 2: serionjq7k (전자결제) — 결제 대기 (33만원 보류, 4/22 전)
- 사업자번호: 7541302876
- 전화 확인 필요: 1544-7772 (MID 통합 가능?)

## GitHub 토큰
- 갱신 완료 (2026-03-24)
- 만료: 2026-06-22 (90일)
- 세리온(223.130.162.133) + Foundry(175.45.200.162) 둘 다 교체 완료

## Snowflake 해커톤 2026
- 참가 신청 완료 (3/23)
- 과제 제출: 4/1~4/12, 결선: 4/29
- 전략: 비즈니스 트랙 + Foundry AI 회의실 + Snowflake API

## NCP 서버 3대
- Foundry: 175.45.200.162 (s2-g3a, 운영 중)
- 세리온: 223.130.162.133 (s2-g3a, 운영 중)
- 구서버: 49.50.12.8.40 (mi1-g3, 반납 대상! 월 ~5만원 절약)

### 기타
- [x] Sonnet 모델 활성화 (3/21)
- [ ] 모두의창업 공급기업 신청 (4/1 마감!)
- [ ] CHAIN-G 서류 (4/3 마감)
- [ ] 토스 심사 회신 (메일 답장!)
- 마케팅 전략: /Users/mark/Desktop/세리온 전략/Foundry_마케팅.md
