...
### Phase 10 ❌ Lovable 핵심 (4/1~4/14)
- [ ] **Agent Mode** — 자율 디버깅+웹검색+연쇄수정 (F6 루프 → 범용 Agent 확장)
- [ ] **Chat Mode 고도화** — 컨텍스트 유지, 질문→확인→실행 대화
- [ ] **Visual Edits** — 미리보기 클릭→해당 요소 수정 (iframe postMessage+컴포넌트 매핑)
- [ ] **GitHub 연동** — 자동 commit+push, 코드 소유권
- [ ] **빌드 성공률 95%+** — AST 파싱 + TS 사전컴파일
- [ ] **⭐ 프로젝트 메모리 시스템** — 세리온 MEMORY.md 패턴 적용, 고객이 재접속해도 AI가 기억 (ProjectMemory+UserMemory DB, Haiku 자동 요약, 시스템 프롬프트 주입)

### Phase 11 ❌ 프로 기능 (4/14~4/28)
- [ ] **결제 실연동** — 토스 실키 + 크레딧 자동충전 + 세금계산서
- [ ] **호스팅 월 과금 (MRR!)** — 앱별 호스팅 + 방문자 대시보드
- [ ] **도메인** — *.foundry.ai.kr 유지, 커스텀 원하면 코드 다운로드+배포 가이드
- [ ] **AI 이미지 생성** — Gemini API (Imagen 3 / 나노바나나) 1순위, 비싸면 Flux 대체
- [ ] **인증 플로우 자동생성** — 카카오/구글 소셜 + RBAC 자동
- [ ] **⭐ AI 회의실 (/meeting)** — 멀티 AI 브레인스토밍 (GenSpark 스타일, Foundry 내장)
  - 2단계 요금: ⚡스탠다드(Sonnet+4o+Pro, 300cr) / 🔥프리미엄(Opus+o3+Ultra, 1,500cr)
  - 프리셋: 사업계획서 평가 / 시장 분석 / 아이디어 검증 / IR 피드백 / 경쟁사 분석
  - "이걸로 앱 만들기" → /start 연결 (분석 결과 자동 주입)
- [ ] **⭐ 앱 생성 스마트 분석** — 대표 비법 자동화 (Gemini 시장조사→GPT 벤치마크→Claude 설계)
  - 질문지 완료 후 선택지: [🧠 스마트 분석 후 생성] / [⚡ 바로 생성]
...
- 최종 커밋: bf59b53 (2026-03-22)
- 코드 감사 17건 수정 + 교차검증 완료
- 벤치마크: Lovable (120% 목표, 70%에 만족 안 한다)
