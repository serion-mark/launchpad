...
다음 작업 진행해줘. memory/ 폴더 전부 참고.

## 필수 참조 파일
- memory/MEMORY.md — 전체 프로젝트 상태 + 로드맵 + 핵심 파일 경로
- memory/CODE_AUDIT_2026-03-22.md — 코드 감사 보고서 (17건 수정완료, 6건 추후)
- memory/STRATEGY_REPORT_2026-03-21.md — 사업 전략 보고서
- memory/TARGET_APPS_2026-03-21.md — 타겟 앱 분석
- memory/DIRECTION_2026-03-21.md — 기술 방향성

## 현재 상태 (2026-03-22)
- Phase 1~8.5 전부 완료, 배포됨: https://foundry.ai.kr
- 최종 커밋: 21bd186 (2026-03-22)
- 코드 감사 17건 수정 + 교차검증 완료 (tsc 0에러, 서버에러 0건, 4페이지 정상)
- GitHub: https://github.com/serion-mark/launchpad (private)
...
## Phase 9 작업 (사업화) — 다음 우선순위
1. 시연 영상 3개 (실제 앱 생성 과정 3분 영상)
2. 사용 후기/케이스 스터디 작성
3. 유료 고객 1호 확보
4. 정부지원사업 커뮤니티 마케팅 (네이버 카페, 단톡방)
5. 모두의창업 서류 (4/1 마감)
6. ERD + API 명세서 자동 생성

## 미수정 기술 부채 (현재 동작 영향 없음, 스케일업 시 처리)
- M1 any타입 정리 / M2 색상토큰 / M3 중복코드 추출
- M4 execSync 비동기 / M7 localStorage SSR 가드 / M8 setTimeout 레이스

## 프로덕션 .env 설정 필요 (신규 환경변수)
- NEXT_PUBLIC_TOSS_CLIENT_KEY — Toss 클라이언트 키 (test_ck_D5GePWvyJnrK0W0k6q8gLzN97Eoq)
- ADMIN_EMAILS — 어드민 이메일 (기본값: admin@serion.ai.kr,mark@serion.ai.kr,mark@foundry.kr)
- SUPABASE_REGION — Supabase 리전 (기본값: ap-northeast-1)
- BUILD_TIMEOUT_MS — 빌드 타임아웃 (기본값: 300000)
...
## 핵심 파일 (수정 시 참고)
- api/src/ai/ai.service.ts — AI 코드 생성 엔진 (1800줄+, 프롬프트+파일생성+품질보정)
- api/src/project/deploy.service.ts — 배포 파이프라인 (5단계 빌드+AI 자동수정)
- api/src/main.ts — NestJS 부트스트랩 + AllExceptionsFilter
- web/src/app/builder/page.tsx — 인터랙티브 빌더 (Lovable 스타일, SSE 스트리밍)
- web/src/app/start/page.tsx — 빌더 플로우 (10개 템플릿→질문지→테마→생성)
- web/src/app/page.tsx — 랜딩 페이지 (히어로+강점+사용법+포트폴리오+가격+CTA)
```
