...
### Phase 5 ⚠️ 코드 품질 (현재)
- [x] F2~F7 코드 개선 전부 완료 (3/21)
- [ ] **E2E 테스트**: Haiku로 신규 앱 생성 → 빌드 → 배포 → 서브도메인 접속 ← **지금 여기**
  - 기존 펫메이트 v2 배포 시도 → 실패 (F2 이전 구버전 코드, 마크다운 오염 15개 에러)
  - 신규 미용실 앱 생성(Haiku, 27파일, 77cr) → 배포 시도 → 실패 (에러 2개 but F6가 못 고침)
  - **F6 버그 발견**: 에러 파일 정규식이 `app/` 경로 못 잡음 → 같은 에러 3회 반복
  - 수정 필요: `aiBuildFix()` 에러 파일 추출 정규식 + 동일 에러 반복 시 전체 TSX 스캔
  - 코드 잘림도 여전 (`sales/page.tsx:69` — F4 continuation 미작동 가능성)
