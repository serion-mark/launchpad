...
Phase 6 마무리해줘. memory/MEMORY.md 참고.
- [완료] Phase 6 코드 작업 8개 (프롬프트+Tailwind+Sonnet+동적라우트+미리보기+스마트선별+사이드바+Static Export)
- [남은 Phase 6 작업 2개]:
  1. 빌더 UI 디자인 개선 — "개발자 MVP" → Lovable 수준 "디자이너 제품" UX
     (현재: 다크 배경, 왼쪽 데모미리보기+오른쪽 채팅 → Lovable처럼 세련된 UI+실시간미리보기 중심)
  2. 범용 템플릿 추가 — "자유롭게 만들기" 템플릿
     현재 6종(미용실/CRM/커머스/O2O/에듀/시설) → +범용 추가
     대상 앱: 포모도로, 가계부, 독서기록, 할일관리, 매칭앱, 커뮤니티, 블로그, 대시보드, 랜딩페이지 등
     Lovable 벤치마킹: 업종 선택 없이 자유롭게 설명 → AI가 알아서 생성
