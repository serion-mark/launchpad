...
## 코드 감사
- **최신 보고서**: `memory/CODE_AUDIT_2026-03-22.md` — Critical 5건 + High 6건 + Medium 12건 (미수정)
...
코드 감사 수정 + Phase 8.5 착수해줘. memory/ 폴더 전부 참고.

## 필수 참조 파일
- memory/MEMORY.md — 전체 프로젝트 상태 + 로드맵
- memory/CODE_AUDIT_2026-03-22.md — ⭐ 코드 감사 보고서 (수정 체크리스트 포함)

## 현재 상태
- Phase 1~8 전부 완료, 배포됨: https://foundry.ai.kr
- 최종 커밋: e789495 (2026-03-21)
- 코드 감사 완료: Critical 5건 + High 6건 + Medium 12건 (미수정)

## Phase 8.5 작업: 코드 감사 수정 (23건)

### 🔴 CRITICAL (5건) — 반드시 먼저 처리
1. **C1** login/page.tsx:16 — useState → useEffect 교체
2. **C2** credits/page.tsx:6 — Toss 키 → NEXT_PUBLIC_TOSS_CLIENT_KEY 환경변수
3. **C3** ai.service.ts — previousErrors 메모리 누수 → 빌드 후 초기화
4. **C4** project.controller.ts:81 — rollback 미구현 엔드포인트 제거
5. **C5** main.ts + app-generator.ts — console.log → Logger 교체

### 🟠 HIGH (6건) — Critical 다음
6. **H1** admin.controller.ts — admin 이메일 환경변수 이동
7. **H2** credit.controller.ts — 크레딧 차감 projectId 소유자 검증
8. **H3** admin/deploy — 빈 catch 블록에 Logger.warn 추가
9. **H4** ai.service.ts:494 — isFreeTria → isFreeTrial 오타
10. **H5** ai.controller.ts — 레거시 generate/modify 엔드포인트 제거
11. **H6** auth.service.ts — Kakao 이메일 fallback 충돌 방지

### 🟡 MEDIUM (12건) — 기술 부채
12~23. any 타입 정리, 색상 토큰화, 중복 코드 추출, execSync 비동기화,
       에러 바운더리, 전역 에러 필터, localStorage 가드, setTimeout 레이스,
       Supabase region 환경변수, 빌드 타임아웃 환경변수, Apple 버튼 정리, aria 속성

## 핵심 파일 경로
- api/src/ai/ai.service.ts — AI 코드 생성 엔진
- api/src/project/deploy.service.ts — 배포 파이프라인
- api/src/project/project.controller.ts — 프로젝트 API
- api/src/auth/auth.service.ts — 카카오 OAuth
- api/src/admin/admin.controller.ts — 어드민
- api/src/credit/credit.controller.ts — 크레딧
- api/src/main.ts — 부트스트랩
- web/src/app/login/page.tsx — 로그인
- web/src/app/credits/page.tsx — 크레딧 충전
- web/src/app/admin/page.tsx — 어드민 대시보드
...
- web/src/app/start/page.tsx — 빌더 플로우 (템플릿선택→질문지→테마→생성)
- web/src/app/builder/page.tsx — 인터랙티브 빌더 (Lovable 스타일)
- web/src/app/components/ — LandingNav, Footer, ChatWidget, AppMockup
- api/src/ai/ai.service.ts — AI 코드 생성 엔진 (프롬프트+파일생성+품질보정)
- api/src/supabase/supabase.service.ts — Supabase 프로비저닝 (프로젝트+Storage)
