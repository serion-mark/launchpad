...
### 7. 이용약관 면책 조항 + 환불 규정 (Gemini 제안 채택)
...
1. 서버 업그레이드:
   ...
   - 변경: NCP s8-g2-h100 (8vCPU, 32GB, 100GB) — 월 352,000원
   - 동시 빌드 5개+ 가능
   - 관리 서버 1대 = 1인 운영에 적합
...
### 9. 디스크 관리 + 프로젝트 저장 정책 — ⚠️ 공급기업 선정 시 실행
**문제**: 앱 1개 빌드 = 50~200MB, 1,000명 × 3개 = 300GB+ (100GB 디스크 부족)
**저장 정책 (사용자에게 안내)**:
...
**디스크 구성**:
   1. NCP 블록스토리지 500GB 추가 (~50,000원/월) — 활성 앱 저장
   2. NCP Object Storage 1TB (~30,000원/월) — 비활성 앱 백업
   3. 자동 정리 크론잡:
      - 매일 새벽 4시 실행
      - 30일 미접속 + "오래 보관" 미설정 + 호스팅 미결제 → 빌드 삭제
      - 코드는 DB Json 필드에 보존 (삭제 안 함)
      - Object Storage에 빌드 백업 후 삭제
   4. 디스크 모니터링: 80% 이상 → 알림 (이메일/슬랙)
