...
### 8. 빌드 큐 시스템 + 서버 확장
**빌드 큐**:
   - 동시 빌드 제한 (서버 스펙에 따라 조절)
   - 대기 중이면 "앞에 N개 작업 중, 약 M분 소요" 안내
   - Bull 또는 Redis 기반 큐
**서버 확장 (공급기업 선정 즉시 실행)**:
   - 현재: NCP s2-g3a (2vCPU, 8GB) 1대 = 동시 빌드 1~2개
   - 확장: 1,000명 동시 가능 수준으로 즉시 업그레이드
   - API 서버: 4vCPU 16GB로 업그레이드
   - 빌드 워커: 별도 서버 2대 (각 2vCPU 8GB)
   - 로드밸런서 추가
   - PM2 클러스터 모드 (API 4개 인스턴스)
   - 예상 비용: 월 36만원 → 한 달 해보고 트래픽 없으면 다시 내림
   - SaaS 바우처 선정 시 서버비 정부 지원 가능
