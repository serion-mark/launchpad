...
export function useAgentStream() {
  ...
  const start = useCallback(
    ...
    writeFileCount: 0,
      ...
      const buildPath = useSdk ? 'agent-build-v2-sdk' : 'agent-build-v2';
    ...
    async (answer: string, attachments: string[] = []) => {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/ai/agent-build-v2/${state.sessionId}/answer`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
  ...
  //   POST /api/ai/agent-build-v2/:sessionId/attachments (multipart/form-data)
  ...
  const uploadAttachment = useCallback(
    async (
      ...
      fd.append('file', file);
      const res = await fetch(
        `${API_BASE}/ai/agent-build-v2/${state.sessionId}/attachments`,
        {
          method: 'POST',
