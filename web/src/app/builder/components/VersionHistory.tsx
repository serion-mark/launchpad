...
export default function VersionHistory({ projectId, onRollback }: Props) {
  ...
  return (
    <div className="bg-[#1e1e26] rounded-xl border border-[#2a2a35] overflow-hidden">
      {/* Header */}
      <button
        className="w-full flex items-center justify-between px-4 py-3 hover:bg-[#23232e] transition-colors"
      ...
      >
        ...
        <span className="text-[#8b8fa3] text-xs">{expanded ? '▲' : '▼'}</span>
      </button>
      ...
      <div className="px-3 pb-3 space-y-1.5">
        ...
        {!expanded && sorted.length > 3 && (
          <button
            className="w-full text-center text-[10px] text-[#8b8fa3] hover:text-[#f2f4f6] py-1"
            onClick={() => setExpanded(true)}
          >
            + {sorted.length - 3}개 더 보기
          </button>
        )}
      </div>
    </div>
