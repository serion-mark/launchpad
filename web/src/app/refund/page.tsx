import LandingNav from '../components/LandingNav';
import Footer from '../components/Footer';

export default function RefundPage() {
  return (
    <div className="min-h-screen bg-[var(--bg-card)] text-[var(--text-primary)]">
      <LandingNav />
      <main className="mx-auto max-w-3xl px-5 py-12 md:py-20">
        <h1 className="mb-8 text-3xl font-bold">환불 정책 및 코드 소유권</h1>
        <div className="prose prose-invert prose-sm max-w-none space-y-6 text-[var(--text-secondary)] leading-relaxed [&_h2]:text-[var(--text-primary)] [&_h2]:text-lg [&_h2]:font-bold [&_h2]:mt-8 [&_h2]:mb-3 [&_strong]:text-[var(--text-primary)]">

          <p className="text-xs text-[var(--text-tertiary)]">시행일: 2026년 3월 21일 | 최종 수정: 2026년 3월 23일</p>

          {/* ── 크레딧 환불 ── */}
          <h2>1. 크레딧 환불</h2>
          <p>1. <strong>7일 이내 미사용:</strong> 충전 후 7일 이내, 크레딧을 전혀 사용하지 않은 경우 100% 전액 환불 가능합니다.</p>
          <p>2. <strong>7일 이내 부분 사용:</strong> 충전 후 7일 이내에 일부 크레딧을 사용한 경우, 미사용 잔여 크레딧에 대해 환불합니다 (결제 수수료 10% 공제).</p>
          <p>3. <strong>7일 경과 후:</strong> 충전 후 7일이 경과한 경우에도 미사용 잔여 크레딧에 대해 환불을 신청할 수 있습니다 (결제 수수료 10% 공제). 단, 전자금융거래법에 따라 60% 이상 사용한 경우 잔액 전액 환급이 가능합니다.</p>
          <p>4. <strong>사용 크레딧:</strong> 사용 시점에 AI API 비용이 발생하므로 이미 사용된 크레딧은 환불 불가합니다.</p>
          <p>5. <strong>무료 크레딧:</strong> 회원가입 보너스 등 무료 지급된 크레딧(무상 선불전자지급수단)은 환불 대상이 아닙니다.</p>
          <p>6. <strong>유효기간:</strong> 크레딧의 유효기간은 충전일로부터 1년이며, 유효기간 만료 30일 전 이메일로 안내합니다. 유효기간 경과 시 잔여 크레딧은 소멸됩니다.</p>
          <p>7. <strong>환급 기한:</strong> 환불 승인 후 3영업일 이내에 원래 결제수단으로 환급합니다.</p>

          {/* ── 모두의 창업 패키지 ── */}
          <h2>2. 모두의 창업 패키지 환불</h2>
          <p>1. 정부사업비 정산 기준에 따릅니다.</p>
          <p>2. 크레딧 미사용분에 대해 정부사업비 기준 환불이 가능합니다.</p>
          <p>3. 세금계산서 취소 및 재발행이 가능합니다.</p>

          {/* ── 독립 패키지 ── */}
          <h2>3. 독립 패키지 환불</h2>
          <p>1. <strong>코드 다운로드 전:</strong> 구매 후 다운로드하지 않은 경우 전액 환불 가능합니다.</p>
          <p>2. <strong>코드 다운로드 후:</strong> 다운로드가 완료된 후에는 환불 불가합니다 (디지털 콘텐츠 특성상).</p>
          <p>3. <strong>산출물 품질 이슈:</strong> 산출물(ERD, API 명세 등)에 중대한 오류가 있는 경우 수정 또는 부분 환불을 검토합니다.</p>

          {/* ── 호스팅 ── */}
          <h2>4. 호스팅 해지</h2>
          <p>1. 호스팅은 월 단위 과금되며, 해지 신청 시 해당 월 말까지 서비스가 유지됩니다.</p>
          <p>2. 해지 후 72시간 이내에 배포된 앱이 삭제됩니다.</p>
          <p>3. 해지 전 코드 다운로드를 권장합니다 (다운로드 크레딧 별도).</p>
          <p>4. 이미 결제된 월 호스팅 비용은 일할 환불하지 않습니다.</p>

          {/* ── 서비스 중도 해지 ── */}
          <h2>5. 서비스 중도 해지</h2>
          <p>1. 회원 탈퇴 시 미사용 크레딧은 위 환불 정책에 따라 처리됩니다.</p>
          <p>2. 탈퇴 후 30일간 계정 복구가 가능하며, 이후 모든 데이터가 영구 삭제됩니다.</p>
          <p>3. 호스팅 중인 앱은 탈퇴 즉시 중단됩니다.</p>

          {/* ── 환불 방법 ── */}
          <h2>6. 환불 방법</h2>
          <p>1. 환불 신청: 이메일(mark@serion.ai.kr)로 접수</p>
          <p>2. 처리 기간: 신청일로부터 영업일 기준 5일 이내</p>
          <p>3. 환불 수단: 원래 결제 수단으로 환불</p>

          {/* ── 코드 소유권 ── */}
          <div className="mt-12 rounded-2xl border border-[var(--toss-blue)]/30 bg-[var(--toss-blue)]/5 p-6">
            <h2 className="!mt-0 text-[var(--toss-blue)]">7. 코드 소유권 보증</h2>
            <p>Foundry는 생성된 코드에 대해 다음을 보증합니다:</p>
            <p><strong>1. 100% 소유권:</strong> 서비스를 통해 생성된 모든 코드의 저작권 및 소유권은 이용자에게 귀속됩니다.</p>
            <p><strong>2. 자유로운 사용:</strong> 이용자는 생성된 코드를 수정, 복제, 배포, 판매할 수 있습니다.</p>
            <p><strong>3. 무제한 인수인계:</strong> ZIP 다운로드한 코드를 외주 개발자, 내부 개발팀 등 누구에게나 전달할 수 있습니다.</p>
            <p><strong>4. 회사 무권리:</strong> 회사(세리온)는 이용자가 생성한 코드에 대해 어떠한 권리도 주장하지 않습니다.</p>
          </div>

          {/* ── 면책 ── */}
          <h2>8. 면책 사항</h2>
          <p>1. AI가 생성한 코드에 포함된 오픈소스 라이브러리는 해당 라이선스를 따릅니다 (MIT, Apache 2.0 등).</p>
          <p>2. 생성된 코드의 버그, 보안 취약점에 대한 책임은 이용자에게 있습니다.</p>
          <p>3. 생성된 코드를 이용한 서비스 운영에서 발생하는 법적 분쟁에 대해 회사는 책임지지 않습니다.</p>

          <p className="mt-10 text-xs text-[var(--text-tertiary)]">
            시행일: 2026년 3월 21일<br />
            최종 수정: 2026년 3월 29일<br />
            세리온 대표: 김형석 | 이메일: mark@serion.ai.kr
          </p>
        </div>
      </main>
      <Footer />
    </div>
  );
}
