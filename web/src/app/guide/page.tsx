        ...
        <div className="mb-8 flex gap-2 overflow-x-auto">
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`whitespace-nowrap rounded-xl px-5 py-2.5 text-sm font-semibold transition-colors ${tab === t.id ? 'bg-[var(--toss-blue)] text-white' : 'bg-[var(--bg-elevated)] text-[var(--text-secondary)] hover:text-white'}`}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {tab === 'start' && (
          <div className="space-y-8">
            <div>
              ...
              <p className="text-[var(--text-secondary)]">3가지 방법으로 나만의 앱을 만들어보세요</p>
            </div>

            {/* 방법 1: AI 대화 */}
            <div className="rounded-2xl border border-[var(--toss-blue)]/30 bg-[var(--toss-blue)]/5 p-6">
              <div className="flex items-start gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[var(--toss-blue)] text-xl font-black text-white">💬</div>
                <div className="flex-1">
                  ...
                  <div className="mb-3 space-y-1 text-xs text-[var(--text-tertiary)]">
                    <p>1. &quot;반려동물 돌봄 매칭 앱 만들어줘&quot; 처럼 자유롭게 입력</p>
                    <p>2. AI가 기능/구조를 설계 → [이 기획으로 앱 만들기] 클릭</p>
                    <p>3. 풀스택 코드 자동 생성 + 24시간 무료 체험 배포</p>
                  </div>
                  <a href="/start" className="inline-block rounded-xl bg-[var(--toss-blue)] px-6 py-2.5 text-sm font-bold text-white hover:bg-[var(--toss-blue-hover)] transition-colors">
                    바로 시작 →
                  </a>
                </div>
              </div>
            ...
            {/* 방법 2: AI 회의실 */}
            <div className="rounded-2xl border border-[var(--toss-purple)]/30 bg-[var(--toss-purple)]/5 p-6">
              <div className="flex items-start gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[var(--toss-purple)] text-xl font-black text-white">🧠</div>
                <div className="flex-1">
                  ...
                  <a href="/meeting" className="inline-block rounded-xl border-2 border-[var(--toss-purple)] px-6 py-2.5 text-sm font-bold text-[var(--toss-purple)] hover:bg-[var(--toss-purple)]/10 transition-colors">
                    회의 시작 →
                  </a>
                </div>
              </div>
            ...
            {/* 방법 3: 사업계획서 */}
            <div className="rounded-2xl border border-[var(--toss-yellow)]/30 bg-[var(--toss-yellow)]/5 p-6">
              <div className="flex items-start gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[var(--toss-yellow)] text-xl font-black text-[#17171c]">📄</div>
                <div className="flex-1">
                  ...
                  <a href="/start" className="inline-block rounded-xl border-2 border-[var(--toss-yellow)] px-6 py-2.5 text-sm font-bold text-[var(--toss-yellow)] hover:bg-[var(--toss-yellow)]/10 transition-colors">
                    사업계획서로 시작 →
                  </a>
                </div>
              </div>
            ...
            {/* 꿀팁 */}
            <div className="rounded-2xl border border-[var(--toss-green)]/20 bg-[var(--toss-green)]/5 p-6">
              <h3 className="mb-2 font-bold text-[var(--toss-green)]">꿀팁</h3>
              <ul className="space-y-1 text-sm text-[var(--text-secondary)]">
                <li>- AI와 대화하는 것은 <b className="text-[var(--toss-green)]">완전 무료</b>입니다</li>
        ...
        {tab === 'manage' && (
          <div className="space-y-8">
            <div>
              ...
              <p className="text-[var(--text-secondary)]">앱을 만든 후 효과적으로 운영하는 방법</p>
            </div>
            <div className="grid gap-6 md:grid-cols-2">
              {[
                ...
                <div key={item.title} className="rounded-2xl border border-[var(--border-primary)] bg-[var(--bg-secondary)] p-6">
                  <div className="mb-3 text-3xl">{item.icon}</div>
                  <h3 className="mb-1 text-lg font-bold">{item.title}</h3>
                  <p className="mb-3 text-sm text-[var(--text-secondary)]">{item.desc}</p>
                  <p className="text-xs text-[var(--text-tertiary)] leading-relaxed">{item.detail}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === 'ai' && (
          <div className="space-y-8">
            <div>
              ...
              <p className="text-[var(--text-secondary)]">Foundry AI를 200% 활용하는 방법 — 카페24/아임웹에서는 불가능한 것들</p>
            </div>
            <div className="rounded-2xl border border-[var(--toss-blue)]/20 bg-[var(--toss-blue)]/5 p-6">
              <h3 className="mb-4 font-bold text-[var(--toss-blue)]">카페24/아임웹과 다른 점</h3>
              <div className="grid gap-4 md:grid-cols-2 text-sm">
                <div className="rounded-xl bg-[var(--bg-secondary)] p-4">
                  <div className="mb-2 font-bold text-[var(--toss-red)]">기존 플랫폼</div>
                ...
                <div className="rounded-xl bg-[var(--bg-secondary)] p-4">
                  <div className="mb-2 font-bold text-[var(--toss-green)]">Foundry</div>
                  <ul className="space-y-1 text-[var(--text-secondary)]">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {[
                ...
                <div key={item.title} className="rounded-xl border border-[var(--border-primary)] bg-[var(--bg-secondary)] p-5">
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text-lg font-bold">{item.icon} {item.title}</span>
                    <span className="rounded-lg bg-[var(--toss-yellow)]/10 px-2 py-0.5 text-[10px] font-bold text-[var(--toss-yellow)]">{item.token}</span>
                  </div>
                  <p className="text-sm text-[var(--text-secondary)] leading-relaxed">{item.desc}</p>
                </div>
              ))}
            </div>
            <div className="rounded-2xl bg-[var(--toss-green)]/10 border border-[var(--toss-green)]/20 p-6 text-center">
              <p className="text-lg font-bold text-[var(--toss-green)] mb-1">사업이 잘 될수록 AI가 더 도와드립니다</p>
              <p className="text-sm text-[var(--text-secondary)]">상품이 많아지고, 고객이 늘어나고, 매출이 커질수록 AI 활용도가 높아집니다</p>
            </div>
          </div>
        )}
        ...
        {tab === 'faq' && (
          <div className="space-y-8">
            <div>
              ...
              <p className="text-[var(--text-secondary)]">궁금한 점이 있으신가요?</p>
            </div>
            ...
            <div className="rounded-2xl border border-[var(--border-primary)] bg-[var(--bg-secondary)] p-6 text-center">
              ...
              <p className="mb-4 text-sm text-[var(--text-secondary)]">빌더 채팅에서 AI에게 직접 물어보세요!</p>
              <a href="/start" className="inline-block rounded-xl bg-[var(--toss-blue)] px-6 py-3 text-sm font-bold text-white hover:bg-[var(--toss-blue-hover)] transition-colors">앱 만들러 가기 →</a>
            </div>
          </div>
        )}
      </div>
